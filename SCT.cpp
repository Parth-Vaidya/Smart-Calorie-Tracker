...
Tracker(string name){
    ...
    void enterMeal(){
        ...
        while (true){
            ...
            if (dailyIntake.find(meal) == dailyIntake.end())dailyIntake[meal] = {0, 0, 0, 0};
            for (int i = 0; i < 4; i++)dailyIntake[meal][i] += macros[i];
    ...
    void displayDailySummary(){
        ...
        for (map<string, vector<int>>::iterator it = dailyIntake.begin(); it != dailyIntake.end(); ++it){
            ...
            totalCal += it->second[0];totalPro += it->second[1];totalCarb += it->second[2];totalFat += it->second[3];
        }
        cout << "------------------------------------\n";
...
void createCourse(const string &counsellorName){
    ...
    out << name << ","<< price << ","<< calories << ","<< protein << ","<< carbs << ","<< desc << ","<< counsellorName << "\n";
    out.close();
